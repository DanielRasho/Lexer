...
state4 := &state{id: "4" , transitions: make(map[Symbol]*state), isFinal: true}

state0.transitions["a"] = state1
state0.transitions["b"] = state3
state1.transitions["10"] = state4
state3.transitions["11"] = state4

return &dfa{ 
startState: state0,
states: []*state{ state0, state1state3state4}, 
}
}
